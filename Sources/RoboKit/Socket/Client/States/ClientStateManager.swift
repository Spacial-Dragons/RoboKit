...
extension TCPClient {
    ...
    /// Consider using startConnection(with:) for messages to leverage TLS security features.
    ...
    /// Function to start the connection to the server with a message.
    /// The state update handler administers the possible NWConnection statuses and calls helper methods accordingly.
    /// This is the recommended method as it automatically uses TLS security features when configured.
    /// - Parameter message: The initial message to send once connection is ready
    public func startConnection(with message: MessageType) async {
        let connectionType = security.useTLS ? "TLS" : "TCP"
